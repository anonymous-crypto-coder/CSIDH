...
for i = m:M
  ...
  Sz={};
  for j = 1:4
    [S{j},H{j},V{j},C(j),Sz{j}] = SemiExhaustiveSearch(mus{i},iotas{i},j,max(2,floor(l(i)/(j+2))), ceil(l(i)/j) + 15, mu);
  end
  [~,jstar] = min(C);
  Sigmas{i} = S{jstar};
  Hs{i} = H{jstar};
...
for i = 1:M
  ...
  for j = 1:length(primeses{i})
    temp = primeses{i}(perms{i});
    temp_prime = temp(j);
    permutedprimes(j) = find(primes == temp_prime);
  end
