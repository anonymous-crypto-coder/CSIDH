...
%Define the vectors entry-by-entry
 for i = 1:n
  c(perm(i)) = sum(Hhat(:,i))*mu(perm(i)) + sum(Vhat(i,:))*iota(perm(i)) + kappa(perm(i));
  d(perm(i)) = mu(perm(i));
 end
...
cvx_begin
  ...
  minimize( c'*b + d'*(m*ones(n,1) - b) + (simba_M-1)*sum(mu) + sum(mu) * prr )
  subject to
...
while sum(log(bhat+1)/log(2)) >= 256
  max_idx = max(find(b == max(b)));
  bhat(max_idx) = bhat(max_idx)-1;
  if sum(log(bhat+1)/log(2)) >= 256
    b=bhat;
  end
end
